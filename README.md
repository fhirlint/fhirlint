...
docker run --rm -v $(pwd):/work ghcr.io/fhirlint/fhirlint:1.14.1 validate /work/fhir/
```
...
```yaml
- uses: fhirlint/fhirlint@v1.14.1
  with:
    path: ./fhir/
    fail-on: error
    severity: warning
...
| format | where |
|---|---|
| `json` | a top-level `meta` object: `{"fhirlint": "1.14.1", "validator": "6.10.4", "fhirVersion": "4.0.1"}` |
...
```
Updates available:
  fhirlint   v1.14.0 → v1.14.1  https://github.com/fhirlint/fhirlint/releases
